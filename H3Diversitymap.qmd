...
This script allows for the easy visualization of species diversity metrics estimated by the obisindicators R package, using species occurrence data from gbif/obis as input data. Diversity is estimated and displayed in hexagonal grid cells, adopting a resolution specified by the user. Maps can be generated in .png format in a range of projections or in an interactive html document (fixed to the default crs:4326 of leaflet).

## **SET RUNTIME PARAMETERS**

```{r}
workdir <- "C:/Users/pdeschepper/Desktop/H3SnakeExample"
input <- "SO_Channichthyidae.txt" #tab delimited txt (gbif export occurrences.txt)
...
crs_target <- 3031 #crs target for png,leaflet will only plot the default 4326 projection
estimator <- "shannon_diversity" # one of richness, shannon_diversity, simpson_diversity, ES, maxp, hill_1, hill_2, hill_inf

# Visuals specs
hex_res <- 4 # between 0 and 15, smaller equals larger polygons
plot_background_map <- TRUE # FALSE or TRUE, plot background yes or no
background_type <- "country_borders" # Options: "osm" for OpenStreetMap, "country_borders" for country borders
osm_res <- 7 # zoom level, proxy for map resolution of OpenStreetMap
output_type <- "png"  # "interactive_html" with leaflet or "png" in chosen projection. If you pick "png" and "osm", it might take a long time to create your figure if you have a large study area.
...
The part first sets the working directory and reads the input tab-delimited txt file. It then prepares the data by adding H3 cell IDs to each occurrence record based on its latitude and longitude. Rows with missing species names are filtered out.
...
setwd(workdir)
occurs <- read.table(file = input, sep = "\t", header = TRUE, fill = TRUE)
